...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class EnvironmentDirector : ITick, ISync, IRenderAnnotations
	{
		...
		void StartEvent(World world)
		{
			...
			if (CurrentEvent == EnvironmentEventType.OilFireSmoke)
				SpawnDecorations(world, info.OilFireActor, 4, false, true);
			else if (CurrentEvent == EnvironmentEventType.HeatMirage)
				SpawnDecorations(world, info.MirageContactActor, 9, true, false);

			TextNotificationsManager.AddSystemLine($"{EventName(CurrentEvent)} ACTIVE: {EventEffect(CurrentEvent)}");
			Log.Write("debug", $"EnvironmentDirector started {CurrentEvent} for {ticksRemaining} ticks.");
		}
		...
		void TickLightning(World world)
		{
			...
			Game.Sound.Play(SoundType.UI, "env-thunder.wav");
		}

		void SpawnDecorations(World world, string actorName, int count, bool allowWater, bool nearCombatants)
		{
			var neutral = world.Players.FirstOrDefault(p => p.NonCombatant);
			if (neutral == null || !world.Map.Rules.Actors.ContainsKey(actorName))
				return;

			var anchors = nearCombatants
				? world.Actors.Where(a => a.IsInWorld && a.OccupiesSpace != null && !a.Owner.NonCombatant).Select(a => a.Location).ToArray()
				: [];
			var candidates = world.Map.AllCells
				.Where(c => world.Map.Contains(c)
					&& (allowWater || world.Map.GetTerrainInfo(c).Type != "Water")
					&& (!nearCombatants || anchors.Any(a =>
					{
						var dx = a.X - c.X;
						var dy = a.Y - c.Y;
						var distanceSquared = dx * dx + dy * dy;
						return distanceSquared >= 25 && distanceSquared <= 225;
					}))
		...
		IEnumerable<IRenderable> IRenderAnnotations.RenderAnnotations(Actor actor, WorldRenderer wr)
		{
			...
			yield return new EnvironmentCardRenderable(CurrentEvent, warning, SecondsRemaining(wr.World), adaptation);
		}

		bool IRenderAnnotations.SpatiallyPartitionable => false;

		sealed class EnvironmentCardRenderable : IRenderable, IFinalizedRenderable
		{
			readonly EnvironmentEventType type;
			readonly bool warning;
			readonly int secondsRemaining;
			readonly int adaptation;

			public EnvironmentCardRenderable(EnvironmentEventType type, bool warning, int secondsRemaining, int adaptation)
			{
				this.type = type;
			...
			public int ZOffset => int.MaxValue;
			...
			{
				var accent = warning ? Color.FromArgb(255, 235, 154, 44) : EventColor(type);
				var x = 12f;
